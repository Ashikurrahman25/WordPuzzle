...
public class WordRegion : MonoBehaviour {
    ...
    public void Load(GameLevel gameLevel)
    {
        ...
        validWords = CUtils.BuildListFromString<string>(this.gameLevel.validWords.ToLower());
    ...
    public void CheckAnswer(string checkWord)
    {
        ...
        LineWord line = lines.Find(x => x.answer.ToLower() == checkWord);
        if (line != null)
        {
            if (!line.isShown)
            {
    ...
    public void HintClick()
    {
        ...
        if (MainController.instance.currentHint < MainController.instance.maxHint)
        {
            DialogController.instance.ShowDialog(DialogType.Quiz);
        }
        else
        {
            Toast.instance.ShowMessage("No hints left!");
        }
